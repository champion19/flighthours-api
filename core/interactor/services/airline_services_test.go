...
func (f fakeAirlineRepo) UpdateAirlineStatus(ctx context.Context, tx output.Tx, id string, status string) error {
...
func TestAirlineService_UpdateAirlineStatus(t *testing.T) {
	...
	t.Run("success => commit", func(t *testing.T) {
		...
		svc := NewAirlineService(fakeAirlineRepo{
			...
			updateStatusFn: func(context.Context, output.Tx, string, string) error {
				return nil
			},
		}, noopLogger{})

		err := svc.UpdateAirlineStatus(ctx, "airline-123", "active")
		if err != nil {
			t.Fatalf("expected no error, got %v", err)
		}
		if !tx.committed {
...
func TestAirlineService_ActivateAirline(t *testing.T) {
	...
	t.Run("success", func(t *testing.T) {
		...
		svc := NewAirlineService(fakeAirlineRepo{
			beginTxFn: func(context.Context) (output.Tx, error) { return tx, nil },
			updateStatusFn: func(_ context.Context, _ output.Tx, _ string, status string) error {
		...
		if receivedStatus != "active" {
			t.Fatalf("expected status 'active', got %q", receivedStatus)
		}
	})
...
func TestAirlineService_DeactivateAirline(t *testing.T) {
	...
	t.Run("success", func(t *testing.T) {
		...
		if receivedStatus != "inactive" {
			t.Fatalf("expected status 'inactive', got %q", receivedStatus)
		}
	})
